...
[업스트림 영문·중문 문서](http://gh.bhee.online/zmk-keyboard-cornix/) · [중문 README](README_zh.md) · [일문 README](README_jp.md)

- Cornix 보드 릴리스: [`v3.0.0`](https://github.com/hitsmaxft/zmk-keyboard-cornix/releases/tag/v3.0.0)
- ZMK 기준: `main`, Zephyr 4.1
...
- 좌우 공통 3ms 디바운스와 30분 미사용 후 Deep Sleep
...
- 선택적 USB 동글, RGB 상태 표시 및 DYA Studio 모듈 지원
- GitHub Actions를 통한 펌웨어 빌드, 릴리스, GitHub Pages 및 키맵 다이어그램 자동화

## 빠른 설치

1. [최신 Release](https://github.com/thsrhwk01/zmk-keyboard-cornix/releases/latest)에서 대상에 맞는 UF2 파일을 내려받습니다.
...
| `cornix_dongle_nosd.uf2` | 동글 | 선택적인 nice!nano 기반 전용 동글 펌웨어 |
...
일반적인 펌웨어 업데이트에는 reset 파일이 필요하지 않습니다. 좌우 역할을 변경하거나 Bluetooth 페어링과 저장 설정을 새로 구성해야 할 때만 사용하십시오.
...
Zephyr 4.1에서는 Cornix 보드가 UF2 부트 모드를 retained memory에 기록하므로 키맵의 `&bootloader` 동작으로 진입할 수 있습니다.
...
Zephyr 4.1에서는 다음과 같이 `//zmk` qualifier가 포함된 보드 이름을 사용합니다.

- `cornix_left//zmk`: 동글 없이 사용하는 왼쪽 central
- `cornix_right//zmk`: 오른쪽 peripheral
- `cornix_ph_left//zmk`: 동글 구성의 왼쪽 peripheral
- `nice_nano//zmk`: 전용 동글 및 nice!nano 설정 초기화

특히 nice!nano를 qualifier 없이 빌드하면 저장 backend가 `CONFIG_SETTINGS_NONE=y`로 선택되어 Bluetooth identity와 split bond가 재부팅 후 유지되지 않을 수 있습니다. 동글과 reset 빌드의 최종 `.config`에는 다음 값이 있어야 합니다.
...
`CONFIG_SETTINGS_NONE=y`는 없어야 합니다.

## 선택 기능

### 전용 동글

`build.yaml`에는 nice!nano 동글, 동글용 왼쪽 peripheral 및 오른쪽 peripheral 빌드가 포함되어 있습니다. 동글 화면은 `cornix_dongle_eyelash`와 `dongle_display`를 사용하며, 화면 하드웨어가 이미 `zephyr,display`를 제공한다면 eyelash shield를 제외해야 합니다.
...
Cornix는 no-SoftDevice 플래시 레이아웃을 사용합니다. 이전 펌웨어나 stock RMK 펌웨어와 전환하거나 UF2 모드에 진입할 수 없다면 [부트로더 복구 안내](bootloader/README.md)를 따르십시오. 서로 다른 좌우 역할이나 플래시 레이아웃을 혼용하지 마십시오.
